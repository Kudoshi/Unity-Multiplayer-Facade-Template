...
public class SteamLobbyService : IServiceLobby
{
    ...
    private void OnPlayerDataChangeHandler(Lobby lobby, Friend friend)
    {
        ...
        NetworkLog.LogDev("[SteamLobby] Player data changed | Player: " + friend.Name);
    ...
    private void OnLobbyPlayerJoinHandler(Lobby lobby, Friend friend)
    {
        ...
        NetworkLog.LogDev("[SteamLobby] Player Join lobby | Player: " + friend.Name);
    }
    ...
    private async void OnPlayerLeaveHandler(Lobby lobby, Friend friend)
    {
        ...
        NetworkLog.LogDev($"[SteamLobby] Player left lobby | Player: {friend.Name}");
    }
    ...
    /// <summary>
    /// Truggers when inviting friend to join lobby
    /// </summary>
    ...
    private void OnLobbyInviteHandler(Friend friend, Lobby lobby)
    {
        ...
        NetworkLog.LogDev($"[SteamLobby] Player invited to lobby | Player: {friend.Name}");
    ...
    public async Task<LobbyEnterResult> JoinLobbyByID(string lobbyID)
    {
        ...
        Lobby[] lobbies = await SteamMatchmaking.LobbyList.WithKeyValue(CustomLobby.KEY_LOBBY_CODE, lobbyID).WithSlotsAvailable(1).RequestAsync();
